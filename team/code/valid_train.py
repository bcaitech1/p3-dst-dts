...
def train(config_root: str):
    ...
    if args.train_from_trained is not None:
        tokenizer, processor, train_features, dev_features = get_stuff(trained_config,train_data, dev_data, slot_meta, ontology)
        trained_config.device = args.device
    ...
    for epoch in range(n_epochs):
        ...
        dom_slot_counter=get_Domain_Slot_distribution_counter(Counter(now_wrong_list))
        o_dom_slot_counter=get_Domain_Slot_Value_distribution_counter(Counter(now_correct_list))
        ...
        with open(f"{task_dir}/wrong_list.pickle", 'wb') as f:
            pickle.dump(wrong_list, f, pickle.HIGHEST_PROTOCOL)
        with open(f"{task_dir}/correct_list.pickle", 'wb') as f:
            pickle.dump(correct_list, f, pickle.HIGHEST_PROTOCOL)

        draw_WrongDS(dom_slot_counter,o_dom_slot_counter,epoch)
